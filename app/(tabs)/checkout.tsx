...
export default function Checkout() {
  ...
  const {
    ...
    guestName, prestadorId
  } = params;

  const amount = Number.isFinite(Number(price)) ? Number(price) : 0;
  // Normalize guestName: Expo Router params can be string | string[] | undefined
  const normalizedGuestName = Array.isArray(guestName) ? guestName[0] : (typeof guestName === 'string' ? guestName.trim() : '');

  console.log('[CHECKOUT] Params:', { prestadorId, guestName, normalizedGuestName });
  ...
  const handleConfirmRequest = async () => {
    ...
    try {
      ...
      // Se o cliente escolheu um prestador específico, direcionar para ele
      const hasSpecificProvider = prestadorId && typeof prestadorId === 'string' && prestadorId.length > 0;
        ...
        .insert([
          {
            ...
            nome_visitante: normalizedGuestName || null,
            prestador_id: hasSpecificProvider ? prestadorId : null,
            ...
            status: hasSpecificProvider ? 'direcionada' : 'pendente',
            valor: finalAmount,
      ...
      // Auto-dispatch apenas para solicitações genéricas (sem prestador escolhido)
